    ...
    if(isset($_SESSION['unique_id'])){
        ...
        if(!empty($classid) && !empty($subject) && !empty($day) && !empty($month) && !empty($year) && !empty($description)){
            if($day == 0 || $month == 0 || $year == 0){
                ...
                $sql2 = mysqli_query($conn, "INSERT INTO tests (classroom_unique_id, test_subject, test_description, test_date) VALUES({$classid}, '{$subject}', '{$description}', '{$testdate}')") or die();
                if($sql2){
                    echo "success";
                }else{
